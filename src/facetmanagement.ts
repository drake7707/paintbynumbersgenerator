...
class Facet {
     ...
     */
    neighbourFacetsIsDirty: boolean = false;
...
export class FacetCreator {
     ...
     */
    static async getFacets(width: number, height: number, imgColorIndices: Uint8Array2D, onUpdate: ((progress: number) => void) | null = null): Promise<FacetResult> {
        ...
        // setup visited mask
        let visited = new BooleanArray2D(result.width, result.height);
     ...
     */
    static buildFacet(facetIndex: number, facetColorIndex: number, x: number, y: number, visited: BooleanArray2D, imgColorIndices: Uint8Array2D, facetResult: FacetResult) {
        let facet = new Facet();
        facet.id = facetIndex;
    ...
    static buildFacetNeighbour(facet: Facet, facetResult: FacetResult) {
        ...
        let uniqueFacets: IMap<boolean> = {}; // poor man's set
...
export class FacetReducer {
     ...
     */
    static async reduceFacets(smallerThan: number, removeFacetsFromLargeToSmall: boolean, colorsByIndex: RGB[], facetResult: FacetResult, imgColorIndices: Uint8Array2D, onUpdate: ((progress: number) => void) | null = null) {
        let count = 0;

        let visitedCache = new BooleanArray2D(facetResult.width, facetResult.height);

        // build the color distance matrix, which describes the distance of each color to each other
        let colorDistances: number[][] = FacetReducer.buildColorDistanceMatrix(colorsByIndex);
    ...
    private static rebuildChangedFacets(visitedArrayCache: BooleanArray2D, facetToRemove: Facet, imgColorIndices: Uint8Array2D, facetResult: FacetResult) {
        ...
        let changedNeighboursSet: IMap<boolean> = {};
    ...
    private static buildColorDistanceMatrix(colorsByIndex: RGB[]) {
        ...
        let colorDistances: number[][] = new Array(colorsByIndex.length);
...
export class FacetBorderTracer {
     ...
     */
    static async buildFacetBorderPaths(facetResult: FacetResult, onUpdate: ((progress: number) => void) | null = null) {

        let count = 0;

        var borderMask = new BooleanArray2D(facetResult.width, facetResult.height);
        ...
        for (let fidx: number = 0; fidx < facetProcessingOrder.length; fidx++) {
            ...
            if (f != null) {
                ...
                let xWall = new BooleanArray2D(facetResult.width + 1, facetResult.height + 1);
                let yWall = new BooleanArray2D(facetResult.width + 1, facetResult.height + 1);
    ...
    private static getPath(pt: PathPoint, facetResult: FacetResult, f: Facet, borderMask: BooleanArray2D, xWall: BooleanArray2D, yWall: BooleanArray2D) {
        ...
        while (!finished) {
            ...
            let possibleNextPoints: PathPoint[] = [];
            ...
            if (pt.orientation == OrientationEnum.Left) {
                ...
                if (((pt.y - 1 >= 0 && facetResult.facetMap.get(pt.x, pt.y - 1) != f.id) // top exists and is a neighbour facet
                    ...
                    let nextpt = new PathPoint(new Point(pt.x, pt.y), OrientationEnum.Top);
                ...
                if (((pt.y + 1 < facetResult.height && facetResult.facetMap.get(pt.x, pt.y + 1) != f.id) // bottom exists and is a neighbour facet
                    ...
                    let nextpt = new PathPoint(new Point(pt.x, pt.y), OrientationEnum.Bottom);
                    possibleNextPoints.push(nextpt);
                }
                ...
                if (pt.y - 1 >= 0 // top exists
                    ...
                    let nextpt = new PathPoint(new Point(pt.x, pt.y - 1), OrientationEnum.Left);
                    possibleNextPoints.push(nextpt);
                }
                // check downwards
                //   +---x---+ 
                ...
                if (pt.y - 1 >= 0 && pt.x - 1 >= 0 // there is a left upwards
                    ...
                    let nextpt = new PathPoint(new Point(pt.x - 1, pt.y - 1), OrientationEnum.Bottom);
                    possibleNextPoints.push(nextpt);
                }
                ...
                if (pt.y + 1 < facetResult.height && pt.x - 1 >= 0 // there is a left downwards
                    ...
                    let nextpt = new PathPoint(new Point(pt.x - 1, pt.y + 1), OrientationEnum.Top);
                    possibleNextPoints.push(nextpt);
                }
            ...
            // same in the other orientations 
                ...
                if (((pt.x - 1 >= 0
                    ...
                    let nextpt = new PathPoint(new Point(pt.x, pt.y), OrientationEnum.Left);
                    possibleNextPoints.push(nextpt);
                }
                ...
                if (((pt.x + 1 < facetResult.width
                    ...
                    let nextpt = new PathPoint(new Point(pt.x, pt.y), OrientationEnum.Right);
                    possibleNextPoints.push(nextpt);
                }
                ...
                if (pt.x - 1 >= 0
                    ...
                    let nextpt = new PathPoint(new Point(pt.x - 1, pt.y), OrientationEnum.Top);
                    possibleNextPoints.push(nextpt);
                }
                // check rightwards
                if (pt.x + 1 < facetResult.width
                ...
                if (pt.y - 1 >= 0 && pt.x - 1 >= 0 // there is a left upwards
                    ...
                    let nextpt = new PathPoint(new Point(pt.x - 1, pt.y - 1), OrientationEnum.Right);
                    possibleNextPoints.push(nextpt);
                }
                // check right upwards
                if (pt.y - 1 >= 0 && pt.x + 1 < facetResult.width // there is a right upwards
                    ...
                    let nextpt = new PathPoint(new Point(pt.x + 1, pt.y - 1), OrientationEnum.Left);
                    possibleNextPoints.push(nextpt);
                }
                ...
                if (pt.y - 1 >= 0
                    ...
                    let nextpt = new PathPoint(new Point(pt.x, pt.y - 1), OrientationEnum.Right);
                    possibleNextPoints.push(nextpt);
                }
                // check downwards
                if (pt.y + 1 < facetResult.height
                ...
                if (pt.x - 1 >= 0
                    ...
                    let nextpt = new PathPoint(new Point(pt.x - 1, pt.y), OrientationEnum.Bottom);
                    possibleNextPoints.push(nextpt);
                }
                // check rightwards
                if (pt.x + 1 < facetResult.width
        ...
        for (let pt of path) {
            switch (pt.orientation) {
                case OrientationEnum.Left: xWall.set(pt.x, pt.y, false); break;
                case OrientationEnum.Top: yWall.set(pt.x, pt.y, false); break;
                case OrientationEnum.Right: xWall.set(pt.x + 1, pt.y, false); break;
                case OrientationEnum.Bottom: yWall.set(pt.x, pt.y + 1, false); break;
            }
        }
        return path;
    }
...
export class FacetBorderSegmenter {
     ...
     */
    static async buildFacetBorderSegments(facetResult: FacetResult, nrOfTimesToHalvePoints: number = 2, onUpdate: ((progress: number) => void) | null = null) {
        ...
        let segmentsPerFacet: (PathSegment | null)[][] = FacetBorderSegmenter.prepareSegmentsPerFacet(facetResult);
    ...
    private static prepareSegmentsPerFacet(facetResult: FacetResult) {
        let segmentsPerFacet: (PathSegment | null)[][] = new Array(facetResult.facets.length);
     ...
     */
    private static reduceSegmentComplexity(facetResult: FacetResult, segmentsPerFacet: (PathSegment | null)[][], nrOfTimesToHalvePoints: number) {
        for (let f of facetResult.facets) {
            if (f != null) {
     ...
     */
    private static async matchSegmentsWithNeighbours(facetResult: FacetResult, segmentsPerFacet: (PathSegment | null)[][], onUpdate: ((progress: number) => void) | null = null) {
...
export class FacetLabelPlacer {
     ...
     */
    static async buildFacetLabelBounds(facetResult: FacetResult, onUpdate: ((progress: number) => void) | null = null) {
        ...
        for (let f of facetResult.facets) {
            if (f != null) {
                ...
                for (let neighbourIdx of f.neighbourFacets!) {
                    ...
                    let fallsInside: boolean = FacetLabelPlacer.doesNeighbourFallInsideInCurrentFacet(neighbourPath, f, onlyOuterRing);

                    if (fallsInside)
                        polyRings.push(neighbourPath);
                }
                let result = polylabel(polyRings);
    ...
    private static doesNeighbourFallInsideInCurrentFacet(neighbourPath: Point[], f: Facet, onlyOuterRing: Point[][]) {
        ...
        if (fallsInside) {
            ...
            for (let i: number = 0; i < neighbourPath.length && fallsInside; i++) {
                let distance = pointToPolygonDist(neighbourPath[i].x, neighbourPath[i].y, onlyOuterRing);
                if (distance < 0) {
                    // falls outside
                    fallsInside = false;
                }
