...
function fillCore(x: number, y: number, width:number,height:number,visited: ((i: number, j: number) => boolean), setFill: ((i: number, j: number) => void)) {
    ...
    do {
        let rowLength = 0, sx = x; // keep track of how long this row is. sx is the starting x for the main scan below
        ...
        if (lastRowLength != 0 && visited(x, y)) // if this is not the first row and the leftmost cell is filled...
        {
            do {
        ...
        if (rowLength < lastRowLength) {
            for (let end = x + lastRowLength; ++sx < end;) // 'end' is the end of the previous row, so scan the current row to
        ...
        {
            for (let ux = x + lastRowLength; ++ux < sx;) // sx is the end of the current row
            {
